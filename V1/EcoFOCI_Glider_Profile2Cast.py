...
def find_sharp_grad(xdf, thresh=-1.0):
    ...
    upper_depth_index = (
        np.where(xdf.depth == upper_depth)[0] - 1
    )  # make shallower by one
    ...
    lower_depth = xdf.depth[bottom_depth_index[0] :][
        dtdz[bottom_depth_index[0] :] < dtdz_up_thresh
    ][0]
...
def find_max_inversion(temperature=None, salinity=None, pressure=None):
    sigmat = sw.dens(s=salinity, t=temperature, p=pressure) - 1000.0
...
for divenum in sorted(dives):
    ...
    try:
        ...
        (
            ...
            lower_depth,
            lower_depth_index,
        ) = find_sharp_grad(xdf, thresh=-1.0)

        downcast_trans = np.where(
            (xdf.depth[0 : bottom_depth_index[0] + 1] >= upper_depth)
            & (xdf.depth[0 : bottom_depth_index[0] + 1] <= lower_depth)
        )[0]
        ...
        if args.plots:
            ...
            plt.plot(
                [
                    xdf.temperature[upper_depth_index[0]],
                    xdf.temperature[bottom_depth_index[0]],
                    xdf.temperature[lower_depth_index[-1]],
                ],
                [
                    xdf.depth[upper_depth_index[0]],
                    xdf.depth[bottom_depth_index[0]],
                    xdf.depth[lower_depth_index[-1]],
                ],
                "ko",
            )
            ...
            plt.plot(
                [
                    xdf.salinity[upper_depth_index[0]],
                    xdf.salinity[bottom_depth_index[0]],
                    xdf.salinity[lower_depth_index[-1]],
                ],
                [
                    xdf.depth[upper_depth_index[0]],
                    xdf.depth[bottom_depth_index[0]],
            ...
            plt.savefig(
                "images/pre_profile_corr_" + divenum + ".png",
                bbox_inches="tight",
                dpi=(300),
            )
            plt.close()
        ...
        sprime = rescale(
            tprime,
            [xdf.salinity[upper_depth_index][0], xdf.salinity[lower_depth_index][-1]],
        )
        """-----------------------------------------------------------------"""
        ...
        if args.plots:
            ...
            ax1 = fig.add_subplot(121)
            plt.plot(
                xdf.temperature[0 : upper_depth_index[0] + 1],
                xdf.depth[0 : upper_depth_index[0] + 1],
                "r.-",
            )
            plt.plot(
                xdf.temperature[bottom_depth_index[0] : lower_depth_index[-1] + 1],
                xdf.depth[bottom_depth_index[0] : lower_depth_index[-1] + 1],
                "m.-",
            )
            ...
            ax1 = fig.add_subplot(122)
            plt.plot(
                xdf.salinity[0 : upper_depth_index[0] + 1],
                xdf.depth[0 : upper_depth_index[0] + 1],
                "b.-",
            )
            plt.plot(
                xdf.salinity[bottom_depth_index[0] : lower_depth_index[-1] + 1],
                xdf.depth[bottom_depth_index[0] : lower_depth_index[-1] + 1],
                "c.-",
            )
            ...
            plt.savefig(
                "images/post_profile_corr_" + divenum + ".png",
                bbox_inches="tight",
                dpi=(300),
            )
            plt.close()
        ...
        temp_cor = np.hstack(
            (
                xdf.temperature[0 : upper_depth_index[0] + 1],
                xdf.temperature[downcast_trans],
                xdf.temperature[bottom_depth_index[0] : lower_depth_index[-1] + 1],
            )
        )
        press_cor = np.hstack(
            (
                xdf.depth[0 : upper_depth_index[0] + 1],
                xdf.depth[downcast_trans],
                xdf.depth[bottom_depth_index[0] : lower_depth_index[-1] + 1],
            )
        )
        fluor_cor = np.hstack(
            (
                xdf.wlbb2fl_sig695nm_adjusted[0 : upper_depth_index[0] + 1],
                xdf.wlbb2fl_sig695nm_adjusted[downcast_trans],
                xdf.wlbb2fl_sig695nm_adjusted[
                    bottom_depth_index[0] : lower_depth_index[-1] + 1
                ],
            )
        )

        pdfa = pd.DataFrame(
            np.stack((sal_cor, temp_cor, press_cor, fluor_cor)).T,
            columns=["Salinity", "Temperature", "Pressure", "ChlorophyllA"],
        )
        ...
        xdfa = xa.Dataset(
            pdfa,
            coords={
                "latitude": xdf.latitude[bottom_depth_index[0]],
                "longitude": xdf.longitude[bottom_depth_index[0]],
                "time": xdf.time[bottom_depth_index[0]],
            },
        )
        ...
        onem_dfa = xa.Dataset(
            dfg[["ChlorophyllA", "Temperature", "Salinity"]],
            coords={
                "latitude": xdf.latitude[bottom_depth_index[0]],
                "longitude": xdf.longitude[bottom_depth_index[0]],
                "time": xdf.time[bottom_depth_index[0]],
        ...
        invval, invind = find_max_inversion(
            salinity=onem_dfa.Salinity.values,
        ...
        if args.plots:
            ...
            plt.savefig(
                "images/basic_profile_" + divenum + ".png",
                bbox_inches="tight",
                dpi=(300),
            )
            plt.close()
        ...
        pdfa = pd.DataFrame(
            np.stack((sal_down, temp_down, press_down, fluor_down)).T,
            columns=["Salinity", "Temperature", "Pressure", "ChlorophyllA"],
        )
        pdfa.set_index("Pressure", inplace=True)
        pdfa.sort_index(inplace=True)
        ...
        xdfa = xa.Dataset(
            pdfa,
            coords={
                "latitude": xdf.latitude[0],
                "longitude": xdf.longitude[0],
                "time": xdf.time[0],
            },
        )
        ...
        # find inversions
        invval, invind = find_max_inversion(
            salinity=down_dfa.Salinity.values,
        ...
        pdfa = pd.DataFrame(
            np.stack((sal_up, temp_up, press_up, fluor_up)).T,
            columns=["Salinity", "Temperature", "Pressure", "ChlorophyllA"],
        )
        pdfa.set_index("Pressure", inplace=True)
        pdfa.sort_index(inplace=True)
        ...
        xdfa = xa.Dataset(
            pdfa,
            coords={
                "latitude": xdf.latitude[-1],
                "longitude": xdf.longitude[-1],
                "time": xdf.time[-1],
            },
        )
        ...
        # find inversions
        invval, invind = find_max_inversion(
            salinity=up_dfa.Salinity.values,
